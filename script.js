...
function caesarCipher(string, shift = 1) {
  ...
  inputInArray.forEach(function (value, i) {
    if (value.match(/^[a-zA-Z]*$/))
      inputInArray[i] = nextCharacter(value, shift);
  });
...
function nextCharacter(character, shift) {
  ...
  if (
    (character.match(/^[A-Z]*$/) && nextUnicode > 90) ||
    (character.match(/^[a-z]*$/) && nextUnicode > 122)
  )
    nextUnicode -= 26;
  if (
    (character.match(/^[A-Z]*$/) && nextUnicode < 65) ||
    (character.match(/^[a-z]*$/) && nextUnicode < 97)
  )
    nextUnicode += 26;
